    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", padding: 20 }}>
        <View style={{ backgroundColor: "white", borderRadius: 16, padding: 20 }}>
          ...
          {!optedOut && (
            <Text style={{ fontSize: 14, fontWeight: "700", color: colors.brand, marginBottom: 10 }}>
              {daysLeft} day{daysLeft === 1 ? "" : "s"} left this month
            </Text>
          )}
          <Text style={{ fontSize: 15, color: colors.ink, opacity: 0.75, lineHeight: 19, marginBottom: 8 }}>
          ...
          {generic ? null : optedOut ? (
            ...
            <View style={{ backgroundColor: colors.brandLight, borderRadius: 12, padding: 12, marginBottom: 16 }}>
              <Text style={{ fontSize: 15, fontWeight: "600", color: colors.brand }}>🎉 This post is entered!</Text>
              <Text style={{ fontSize: 14, color: colors.muted, marginTop: 4 }}>
            ...
            <View style={{ marginBottom: 16 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
                <Text style={{ fontSize: 14, color: colors.muted }}>{likeCount} / {THRESHOLD} likes</Text>
                <Text style={{ fontSize: 14, color: colors.muted }}>{Math.max(0, THRESHOLD - likeCount)} to go</Text>
              </View>
