  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 48 }} keyboardShouldPersistTaps="handled">
        ...
        <Text style={{ fontSize: 27, fontWeight: "800", color: colors.ink, textAlign: "center", lineHeight: 32, marginBottom: 10 }}>
          Learn real life skills from{" "}
          <Text style={{ backgroundColor: colors.highlight }}>experts in their field</Text>
        </Text>
        <Text style={{ fontSize: 18, color: colors.muted, textAlign: "center", marginBottom: 20 }}>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginBottom: 28 }}>
          <View style={{ flex: 1, borderRadius: 16, overflow: "hidden", backgroundColor: "white", shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 6, elevation: 2 }}>
            <View style={{ aspectRatio: 1 }}>
              ...
              <View style={{ position: "absolute", left: 8, top: 8, borderRadius: 999, backgroundColor: "white", paddingHorizontal: 8, paddingVertical: 3 }}>
                <Text style={{ fontSize: 13, fontWeight: "700", color: colors.music, textTransform: "uppercase" }}>Music</Text>
              </View>
            </View>
            <Text style={{ fontSize: 15, fontWeight: "500", padding: 8, lineHeight: 14 }}>
          ...
          <View style={{ flex: 1, borderRadius: 16, overflow: "hidden", backgroundColor: "white", shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 6, elevation: 2 }}>
            <View style={{ aspectRatio: 1 }}>
              ...
              <View style={{ position: "absolute", left: 8, top: 8, borderRadius: 999, backgroundColor: "white", paddingHorizontal: 8, paddingVertical: 3 }}>
                <Text style={{ fontSize: 13, fontWeight: "700", color: colors.art, textTransform: "uppercase" }}>Art</Text>
              </View>
            </View>
            <Text style={{ fontSize: 15, fontWeight: "500", padding: 8, lineHeight: 14 }}>
      ...
      {resetSent ? (
        <Text style={{ color: "#15803D", fontSize: 16, textAlign: "right", marginTop: -6, marginBottom: 8 }}>
      ...
      <Text style={{ fontSize: 24, fontWeight: "800", color: colors.ink, marginBottom: 6 }}>
        2 subjects. Build real world skills.
      </Text>
      <Text style={{ fontSize: 17, color: colors.muted, marginBottom: 16, lineHeight: 18 }}>
      ...
      {SUBJECT_DETAILS.map((s) => (
        ...
        >
          ...
          <Text style={{ fontSize: 19, fontWeight: "800", color: colors.ink, marginBottom: 8 }}>
            {s.name}
          </Text>
          {s.items.map((item) => (
            <Text key={item} style={{ fontSize: 16, color: colors.muted, marginBottom: 4, lineHeight: 16 }}>
              → {item}
            </Text>
          ))}
          <View style={{ alignSelf: "flex-start", backgroundColor: s.tagBg, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4, marginTop: 8 }}>
            <Text style={{ fontSize: 15, fontWeight: "600", color: s.tagText }}>{s.tag}</Text>
          </View>
        </View>
      ))}
      <View style={{ backgroundColor: "white", borderRadius: 16, borderTopWidth: 4, borderTopColor: colors.highlight, padding: 16, marginBottom: 28 }}>
        <Text style={{ fontSize: 17, color: colors.ink, lineHeight: 19 }}>
      ...
      <Text style={{ fontSize: 24, fontWeight: "800", color: colors.ink, marginBottom: 6 }}>Simple as 1, 2, 3</Text>
      <Text style={{ fontSize: 17, color: colors.muted, marginBottom: 16, lineHeight: 18 }}>
        Get started in minutes, and keep building new skills over time.
      </Text>
      {STEPS.map((step) => (
        <View key={step.n} style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 29, fontWeight: "900", color: "rgba(23,19,15,0.15)", marginBottom: 2 }}>{step.n}</Text>
          <Text style={{ fontSize: 18, fontWeight: "700", color: colors.ink, marginBottom: 2 }}>{step.title}</Text>
          <Text style={{ fontSize: 16, color: colors.muted, lineHeight: 17 }}>{step.blurb}</Text>
        </View>
      ))}

      {/* Pricing */}
      <View style={{ backgroundColor: colors.sectionLavender, borderRadius: 20, padding: 20, marginBottom: 24, marginTop: 8 }}>
        <Text style={{ fontSize: 22, fontWeight: "800", color: colors.ink, marginBottom: 6 }}>Unlock the full lesson library</Text>
        <Text style={{ fontSize: 17, color: colors.muted, marginBottom: 14, lineHeight: 18 }}>
        ...
        <View style={{ backgroundColor: "white", borderRadius: 16, padding: 18 }}>
          ...
          {PRICING_FEATURES.map((f) => (
            <View key={f} style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
              ...
              <Text style={{ fontSize: 17, color: colors.ink, flex: 1 }}>{f}</Text>
            </View>
          ))}
      ...
      <View style={{ backgroundColor: colors.sectionMint, borderRadius: 20, padding: 20, marginBottom: 24 }}>
        ...
        <Text style={{ fontSize: 35, fontWeight: "900", color: colors.ink, marginBottom: 4 }}>AU$1,000</Text>
        <Text style={{ fontSize: 19, fontWeight: "800", color: colors.ink, marginBottom: 6, lineHeight: 22 }}>
          Every month, for the community&apos;s most-loved post
        </Text>
        <Text style={{ fontSize: 15, fontWeight: "700", color: colors.brand, backgroundColor: "white", alignSelf: "flex-start", borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4, marginBottom: 10, overflow: "hidden" }}>
        ...
        {PRIZE_STEPS.map((step) => (
          <View key={step.n} style={{ flexDirection: "row", gap: 10, marginBottom: 10 }}>
            <View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: colors.ink, alignItems: "center", justifyContent: "center" }}>
              <Text style={{ color: "white", fontSize: 15, fontWeight: "700" }}>{step.n}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 17, fontWeight: "700", color: colors.ink, marginBottom: 1 }}>{step.t}</Text>
              <Text style={{ fontSize: 16, color: colors.muted, lineHeight: 16 }}>{step.d}</Text>
            </View>
          </View>
        ))}
      </View>
      ...
      {/* FAQ */}
      <Text style={{ fontSize: 24, fontWeight: "800", color: colors.ink, marginBottom: 14 }}>Common questions</Text>
      <View style={{ backgroundColor: "white", borderRadius: 16, marginBottom: 24, overflow: "hidden" }}>
        {FAQS.map((f, i) => (
          <View key={f.q} style={{ borderTopWidth: i === 0 ? 0 : 1, borderTopColor: colors.line + "1A" }}>
            <TouchableOpacity
              onPress={() => setOpenFaq(openFaq === i ? null : i)}
              style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 16, gap: 12 }}
            >
              <Text style={{ flex: 1, fontSize: 18, fontWeight: "700", color: colors.ink }}>{f.q}</Text>
              <Text style={{ fontSize: 20, color: colors.muted }}>{openFaq === i ? "−" : "+"}</Text>
            </TouchableOpacity>
            {openFaq === i && (
              <Text style={{ paddingHorizontal: 16, paddingBottom: 16, fontSize: 17, color: colors.muted, lineHeight: 18 }}>
                {f.a}
              </Text>
            )}
          </View>
